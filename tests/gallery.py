...
# renders example gallery; you can run this directly but the intent is to use
# it as a worker script in a container. see ../Containerfile for details
...
def example(of):
    def decorator(scene):
        name = of if isinstance(of, str) else of.__name__
        _examples.setdefault(name, []).append(scene)
        return scene
    return decorator
...
class BlocksExample1(Scene):
    def construct(self):
        from hashlib import sha256
        msg = b'aaaa'
        img = sha256(msg).digest()

        input_block = Block(msg, c_fills=C_PT, block_size=None).shift(UP*1.5)
        output_blocks = Blocks(img, c_fills=C_CT, pad=False).shift(DOWN*1.5)
        func_box = FuncBox.with_arrows(
            (input_block, DOWN, UP),
            (output_blocks, DOWN, UP),
            "H",
        )
        self.add(input_block, output_blocks, func_box)
...
if __name__ == "__main__":
    # this script is kind of an ugly hack in need of a cleanup
    # but, nevertheless, it works, so please don't judge it too harshly :-)
    import shutil
    import os
    import os.path
    import inspect
    ...
    #            else:
    #                os.remove(rel_path)
    shutil.rmtree(os.path.join("examples", "renders"))
    ...
    def _write_scenes_to_file(outfile, scenes):
        ...
        for Scene in scenes:
            ...
            if dest is None:  # avoid re-rendering
                ...
                print(f"[*] {scene.renderer.num_plays=}")
                print(f"[*] {bool(scene.renderer.num_plays)=}")
                if scene.renderer.num_plays > 0:  # at least 1 play() call; this is a video
                    print("[*] Animation detected")
                    fpath = scene.renderer.file_writer.movie_file_path
                else:  # this is an image
                    print("[*] Image detected")
                    fpath = scene.renderer.file_writer.image_file_path
                fname = os.path.basename(fpath)
                dest = os.path.join("examples", "renders", fname)
                rendered[scene_name] = dest
                print(f"[*] {fpath=}")
                print(f"[*] {dest=}")
...
""")
    ...
    for name, scenes in _examples.items():
        fname = name + ".md"
